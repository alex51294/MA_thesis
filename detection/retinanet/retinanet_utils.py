...
def box_nms(bboxes, scores, threshold=0.5, mode='IoU'):
...
def one_hot_embedding(labels, num_classes):
    ...
    num_anchors = labels.size()[0]

    # create empty hull
    one_hot = torch.zeros(num_anchors, num_classes, device=labels.device)

    # set the given class as 1
    one_hot.scatter_(1, labels.unsqueeze(1), 1)

    return one_hot

def one_hot_embedding_batch(batch, num_classes):
    '''Embedding batch to one-hot form.
        Args:
          batch: (LongTensor) batch of class labels, sized [N, #anchors].
          num_classes: (int) number of classes.

        Returns:
          (tensor) encoded labels, sized [N, #anchors, #classes].
        '''

    # get the required dimensions
    num_img, num_anchors = batch.size()

    # create empty hull
    one_hot = torch.zeros(num_img, num_anchors, num_classes,
                          device=batch.device)

    # remove the "ignored labels" (-1) and set them as background
    index = torch.clamp(batch, min=0)

    # set the given class as 1
    one_hot.scatter_(2, index.unsqueeze(2), 1)
